...
function addItem(state, product, domain) {
    const itemIndex = state.findIndex((x) => x.id === product.id);
    if (itemIndex === -1) {
        return [...state, JSON?.parse(JSON.stringify(product, (product.domain = domain)))];
    }
...
export default function wishlistReducer(state = initialState, action) {
    switch (action.type) {
        case WISHLIST_ADD_ITEM:
            return addItem(state, action.product, action.domain);
